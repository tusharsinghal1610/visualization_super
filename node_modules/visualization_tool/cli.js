#!/usr/bin/env node
const [, , ...args] = process.argv;
var readJson = require('read-package-json');
var fs = require('fs');
var currentDir = process.cwd();
var Graph = require("./graph");
var mygraph = new Graph();
var exec = require('child_process').exec;
var semver = require('semver');
var readPackageJson = function (path) {
    return new Promise((resolve, reject) => {
        readJson(path + '/package.json', console.error, false, async function (er, data) {
            if (er) {
                console.log(er);
                console.error("There was an error reading the package.json file")
                return
            }
            else {
                resolve(data);
               // console.log('the package data is', data);
            }
        });
    });
}

// var updateDependency = async(path,package)=>{
//     let count = 10;
//     while(count--){
//         console.log("updated depency");
//     }
//     console.log("updateDependency");
//     return new Promise(async (resolve,reject)=>{
//         let newVersion = semver.inc(package.child.split('@')[1],args[1]);
//         let dependencyData = await readPackageJson(path);
//         console.log(dependencyData);
//         dependencyData.dependencies[package.child.split('@')[0]] = newVersion;
//         console.log(dependencyData);
//         fs.writeFile(path + '/package.json', dependencyData, 'utf8', (err)=>{
//             if(err){
//                 console.log("there was an error in writing the data");
//                 console.log(err);
//             }
//             else{
//                 let command = "cd "+ path + " && git add . && git commit -m updatedDependency" + package.child +" && git push";
//                 exec(command,(error,stdout,stderr)=>{
//                     if(error || stderr){
//                         console.log(error || stderr);
//                     }
//                     console.log(command);
//                     resolve();
//                 });
//             }
//         });  
//     })
// }

var updateDependency = async(path,package)=>{
    return new Promise(async (resolve,reject)=>{
                let command = "cd "+ path + " && npm update " + package.child.split('@')[0] +" && git add . && git commit -m updatedDependency" + package.child +" && git push";
                exec(command,(error,stdout,stderr)=>{
                    if(error || stderr){
                        console.log(error || stderr);
                    }
                    console.log(command);
                    resolve();
                });
            });
        }
        

var updatePackage = async (path,package) => {
    return new Promise(async (resolve,reject)=>{
        if(package.child!=null){
            await updateDependency(path,package);
        }
        let command="cd "+path +" && npm version "
        if (args[1] == 'patch') {
            command = command + "patch";
        }
        else if (args[1] == 'minor') {
            command = command + "minor";
        }
        else if (args[1] == 'major') {
            command = command + "major";
        }
        else {
            console.log("invalid command");
            return;
        }
        if(package.child == null){
            command = command + ' -m "Updated '+ args[1] +' version"';
        }
        else{
            let packageJSON = await readPackageJson(path);
            let gitURL = packageJSON.repository.url.split('+')[1];
            console.log(gitURL);
            let commandgit = "cd " + path + " && git init && git remote add origin "+ gitURL;
            // + " && git checkout master" ;
            command = command + ' -m "Updated due to '+ args[1] +' update on a dependency' + package.child + '"';
            command = commandgit + ' && ' + command;
        }
        exec(command,(error,stdout,stderr)=>{
            if(error || stderr){
                console.log(error||stderr);
                exec("rm -r temptushar"+package.parent,(error,stdout,stderr)=>{
                    console.log("deleted");
                    resolve();
                    })    
            }
            else{
                console.log("successful")
                exec("rm -r temptushar"+package.parent,(error,stdout,stderr)=>{
                console.log("deleted");
                resolve();
                })    
            }
        })
    });
}


var updatePackageGit = async (path,package) => {
    return new Promise(async (resolve,reject)=>{
        if(package.child!=null){
            await updateDependency(path,package);
        }
        let command="cd "+path +" && npm version "
        if (args[1] == 'patch') {
            command = command + "patch";
        }
        else if (args[1] == 'minor') {
            command = command + "minor";
        }
        else if (args[1] == 'major') {
            command = command + "major";
        }
        else {
            console.log("invalid command");
            return;
        }
        if(package.child == null){
            command = command + ' -m "Updated '+ args[1] +' version"';
        }
        else{
            let packageJSON = await readPackageJson(path);
            let gitURL = packageJSON.repository.url.split('+')[1];
            console.log(gitURL);
            //let commandgit = "cd " + path + " && git init && git remote add origin "+ gitURL;
            // + " && git checkout master" ;
            command = command + ' -m "Updated due to '+ args[1] +' update on a dependency' + package.child + '"';
            //command = commandgit + ' && ' + command;
        }
        exec(command,(error,stdout,stderr)=>{
            if(error || stderr){
                console.log(error||stderr);
                exec("rm -r temptushar"+package.parent,(error,stdout,stderr)=>{
                    console.log("deleted");
                    resolve();
                    })    
            }
            else{
                console.log("successful")
                exec("rm -r temptushar"+package.parent,(error,stdout,stderr)=>{
                console.log("deleted");
                resolve();
                })    
            }
        })
    });
}


updateByInstalling = async function (package, path) {
    return new Promise((resolve,reject)=>{
        let command = "cd " + path + "/ && npm i --prefix ./ " + package.parent;
        exec(command,async (error, stdout, stderr) => {
                let newPath = path + "/node_modules/" + package.parent.split('@')[0];
                await updatePackage(newPath,package);
                resolve();
        });
    });
}
updateByInstallingGit = async function (package, path) {
    return new Promise((resolve,reject)=>{
        let command = "cd " + path + "/ && git clone "+ mygraph.nodeInfo.get(package.parent).repository.url.split('+')[1];
        exec(command,async (error, stdout, stderr) => {
                let newPath = path + "/"+ mygraph.nodeInfo.get(package.parent).name + "/";
                console.log(newPath);
                await updatePackageGit(newPath,package);
                resolve();
        });
    });
}
processPackage = async function (path, package) {
    return new Promise((resolve,reject)=>{
        let newPath = path + "/temptushar" + package.parent;
        let command = "cd " + path + "/ && mkdir temptushar" + package.parent;
        exec(command,async (error, stdout, stderr) => {
            await updateByInstallingGit(package, newPath);
            resolve();
        })
    })
}
var mainFun = async function () {
    await mygraph.buildGraph(["visualization_super"]);
    //mygraph.generateNodesAndEdges();
    let mainPackage = await readPackageJson(currentDir);
    let dependents = mygraph.getDependents(mainPackage._id);
    console.log(dependents);
    console.log(mainPackage._id);
    if (args[0] == 'u') {
        await updatePackage(currentDir,{parent:mainPackage._id,child:null});
        dependents.map(async (item) => {
            await processPackage(currentDir, item);
        });
    }
}

mainFun();
